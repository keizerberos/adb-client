...
@Component({
	selector: 'noded-json-info',
	templateUrl: './json-info.component.html',
	styleUrl: './json-info.component.scss',
})

export class JsonInfoComponent implements OnInit, OnChanges {
	@Input() name: any;
	@Input() json: any;
	@Input() data: any;
	@Input() type: any;
	...
	load() {
		if (this.data == undefined) {
			...
			if (typeof valueRaw === 'object' && !Array.isArray(valueRaw) && valueRaw !== null) {
				console.log("load new json1");
				this.format = this.getFormat(valueRaw);
